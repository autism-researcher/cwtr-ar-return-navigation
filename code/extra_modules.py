...
def cwtr_rmse(rate, use_acc=True, use_kin=True, seed=S.SEED):
    ...
    for _ in range(S.M):
        truth=S.gen_truth(r); z,a,sigma=S.gen_obs(truth,rate,r); n=len(z); c=np.ones(n)
        for t in range(n):
            cacc=1/(1+(a[t]/S.A0)**2) if use_acc else 1.0
            s=np.linalg.norm(z[t]-z[t-1])/S.DT if t>0 else 0.0
            ckin=1/(1+(max(0,s-S.VMAX))**2) if use_kin else 1.0
...
def wscore(pos):
    ...
    net=np.linalg.norm(pos[-1]-pos[0]); straight=net/(L+1e-9)
    ang=np.arctan2(d[:,1],d[:,0]); dh=np.abs(np.diff(ang)); dh=np.minimum(dh,2*np.pi-dh)
    cells={tuple(x) for x in np.round(pos/3.0).astype(int)}; revisit=len(pos)-len(cells)
    return (1-straight) + np.var(dh)*0.5 + revisit*0.02

# ---------------- Wandering detection ----------------
r=np.random.default_rng(11)
sp=np.array([wscore(gen_purposeful(r)) for _ in range(250)])
sw=np.array([wscore(gen_wandering(r)) for _ in range(250)])
scores=np.concatenate([sp,sw]); labels=np.array([0]*250+[1]*250)
...
clean=[wscore(gen_purposeful(r,0.08)) for _ in range(200)]  # straight goal-directed
thr=np.percentile(clean,95)
raw_flag=cwtr_flag=0; N=200; r=np.random.default_rng(24)
for _ in range(N):
    ...
    c=np.ones(Tn)
    for t in range(Tn):
        s=np.linalg.norm(z[t]-z[t-1])/1.0 if t>0 else 0.0
        c[t]=np.clip((1/(1+(a[t]/S.A0)**2))*(1/(1+(max(0,s-S.VMAX))**2)),1e-3,1)
...
print(f"raw-fix detector false-trigger    = {100*raw_flag/N:.0f}%")
print(f"CWTR-gated detector false-trigger = {100*cwtr_flag/N:.0f}%")
